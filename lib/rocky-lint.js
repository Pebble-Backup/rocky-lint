...
function printAllDiagonstics(program, fileNameReplacements) {
  ...
  function printDiagonstic(level, d) {
    ...
    console.log("%s %s", colors.cyan(locationString), line.trim());
...
fs.mkdtemp(os.tmpdir(), (err, dir) => {
  ...
  const stats = printAllDiagonstics(program, fileNameReplacements);
